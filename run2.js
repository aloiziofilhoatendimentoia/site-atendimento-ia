...
async function run() {
  ...
  if (agentNode) {
    ...
    if (prompt) {
      ...
      prompt = prompt.replace(/<company>.*?<\/company>/s, '<company>{{ .first().json.nome_clinica }}</company>');
      const newClinicInfo = \<clinic_information>\n    Aqui estao os dados completos e configuracoes da clinica:\n    {{ Puxar_Dados_Clinica.first().json.dados_completos_json }}\n  </clinic_information>\;
  ...
  const res = await req(BASE_URL + '/' + WF_ID, 'PUT', wf);
