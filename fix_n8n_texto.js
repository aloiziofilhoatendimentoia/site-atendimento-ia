...
const req = https.request(options, res => {
  ...
  res.on('end', () => {
    ...
    const node = wf.nodes.find(n => n.name === 'texto');
    if (node) {
      let assignment = node.parameters.assignments.assignments.find(a => a.name === 'Mensagem');
      
      assignment.value = "={{ String($('Webhook').first().json.body?.data?.message?.conversation || $('Webhook').first().json.body?.data?.message?.extendedTextMessage?.text || $('Webhook').first().json.data?.message?.conversation || $('Webhook').first().json.data?.message?.extendedTextMessage?.text || '') }}";
      
      console.log("Updated texto node Mensagem to:", assignment.value);
    }
